...
function CheckinCheckoutCalendar(props: CheckinCheckoutCalendarProps) {
  ...
  const handleSelection = (newbookingDates: any) => {
    const formatDate = (date: any) => {
      const parsedDate = addDays(new Date(date), 1);
    ...
    setDetails?.((prevDetails) => ({
      ...
      bookingDates: {
        ...
        from: formatDate(newbookingDates.from),
        to: formatDate(newbookingDates.to),
      },
    }));
  };
  ...
  return (
    <div className="flex flex-col sm:justify-end md:w-[430px] sm:w-[400px] w-auto h-auto">
      <Popover modal={isModal} onOpenChange={handleOpenChange}>
        ...
        >
          <Button
            ...
            variant={"outline"}
            className={cn(
              "uppercase block items-start text-left text-dark w-full h-[45px] md:h-[50px] font-medium bg-white border border-solid border-border-input  px-4 justify-between rounded-sm  hover:bg-white hover:text-muted-foreground lg:hover:bg-accent lg:hover:text-accent-foreground",
      ...
      {errorMessage && isSubmitted?.step1 && (
